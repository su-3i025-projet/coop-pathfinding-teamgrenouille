    ...
    for j in range(nbPlayers):
        ...
        for t in range (iterations):
            ...
            if(pause[j] == 0): # Si pas de pause
                print("PAUSE 0")
                if reservations[(c,t)] != j and reservations[(c,t)] != -1: # Si case réservée
                    #print(j)
                    print("RESERVATION = 1")
                    collisions.append(c)
                    print("c avant=",c)
                    ...
                    reservations[(c,t+2)] = j
                    '''
                    if len(list)>2 and reservations[(list[-3].etat,t)] == -1 :
                        reservations[(list[-3].etat,t)] = j
                        reservations[(list[-3].etat,t+1)] = j
                    '''
                    chemins[j].append(n)
                    ...
                    if len(list)>2 and reservations[(list[-3].etat,t)] == -1 :
                        ...
                        reservations[(list[-3].etat,t)] = j
                        reservations[(list[-3].etat,t+1)] = j
                        reservations[(list[-3].etat,t+2)] = j
                        if len(list)>3 and reservations[(list[-4].etat,t)] == -1 :
                            reservations[(list[-4].etat,t)] = j
                            reservations[(list[-4].etat,t+1)] = j
                            reservations[(list[-4].etat,t+2)] = j
            ...
            if(pause[j] == 1):
                ...
                if reservations[(c,t)] != j and reservations[(c,t)] != -1 : # Si case réservée
                    ...
                    if fioles[j] not in temp:
                        ...
                        reservations[((next_row,next_col),t+2)] = j
